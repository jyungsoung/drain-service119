...
### `현장 등록` / `현장글 등록`
사용자가 원하는 기본 형태는 사진 중심의 정교한 시공기사가 아니라 **지역 + 서비스 + 짧은 사연 + 해결 내용의 글-only 현장 업데이트**다.

예시 입력:
`현장 등록 경기도 하남시 미사동 싱크대막힘 해결. 사연: 싱크대 물이 거의 내려가지 않아 방문했고 배관 막힘을 해결함.`
...
4. 본문은 과장 없이 1~3개의 짧은 문단으로 정리한다. 장문의 전문 시공기사처럼 늘리지 않는다.
5. `area`, `regionSlug`, `areaHref`, `service`, `serviceHref`, `date`, `slug`를 기존 지역/서비스 구조에 맞춰 자동 연결한다.
6. `app/work-sites/cases-data.ts`에 새 항목을 추가하고 중복 slug를 확인한다.
7. 사진이 없으면 `image`, `imageAlt`, `media`를 생략한다.
8. 별도 브랜치에서 작업하고 `npm run build`를 실행한다.
9. 시공현장 목록·상세페이지·해당 지역 대표페이지·sitemap 연결을 확인한다.
10. 사용자가 같은 요청에서 `배포`까지 명시하면 검수 통과 후 push/PR/병합/Netlify 확인까지 진행한다. `배포`가 없으면 변경 결과만 보고한다.
