...
export function generateStaticParams() { return workCases.map((work) => ({ slug: work.slug })); }
...
export default async function WorkCasePage({ params }: Props) {
  ...
  return <main className="regionPage workCasePage">
    ...
    <article className="workCaseDetail"><header><p className="kicker">{work.area} · {work.service}</p><h1>{work.title}</h1><div className="workCaseMeta"><time dateTime={work.date}>{work.date}</time>{regionHref && <a href={regionHref}>{work.area} 지역 안내 →</a>}{work.serviceHref && <a href={work.serviceHref}>{work.service} 서비스 안내 →</a>}</div></header>{work.image && <figure className="workCaseMainImage"><img src={work.image} alt={work.imageAlt || work.title} /><figcaption>{work.imageAlt || `${work.area} ${work.service} 시공현장`}</figcaption></figure>}<p className="workCaseLead">{work.summary}</p><section className="workCaseStory"><h2>현장 사연과 해결 내용</h2>{work.details.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</section>{work.media && work.media.length > 0 && <section className="workCaseMedia"><div className="workCaseMediaHeading"><p className="kicker">FIELD MEDIA</p><h2>{work.area} {work.service} 현장 사진과 영상</h2></div><div className="workCaseMediaGrid">{work.media.map((media) => <figure key={media.src}>{media.type === "image" ? <img src={media.src} alt={media.alt} loading="lazy" decoding="async" /> : <video controls playsInline preload="metadata" poster={media.poster} aria-label={media.alt}><source src={media.src} type="video/mp4" /><p>{media.alt}</p></video>}<figcaption>{media.caption}</figcaption></figure>)}</div></section>}<div className="workCaseFacts"><DetailList title="현장 증상" items={work.symptoms} /><DetailList title="진단 내용" items={work.diagnosis} /><DetailList title="사용 장비" items={work.equipment} /><DetailList title="점검 결과" items={work.result} /></div><a className="primary compact" href="tel:16681321">비슷한 증상 상담하기</a></article>
    {related.length > 0 && <section className="relatedWorkCases"><div className="regionTitle"><p className="kicker">RELATED WORK</p><h2>관련 시공현장</h2></div><div className="workCaseGrid">{related.map((item) => <a key={item.slug} href={`/work-sites/${item.slug}`}>{item.image && <img src={item.image} alt={item.imageAlt || item.title} />}<span>{item.area} · {item.service}</span><h3>{item.title}</h3><p>{item.summary}</p></a>)}</div></section>}
