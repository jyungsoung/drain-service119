...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${label} 싱크대막힘·변기막힘·하수구막힘 | 응급배관119`;
  const description = `경기도 ${label} 싱크대막힘, 변기막힘, 하수구막힘 원인 점검과 배관 내시경·고압세척 상담. 응급배관119 1668-1321.`;
  return { title, description, keywords:[`${label} 싱크대막힘`,`${label} 변기막힘`,`${label} 하수구막힘`,`${label} 고압세척`], openGraph:{title,description,type:"website",locale:"ko_KR"} };
}
...
export default async function GyeonggiAreaPage({ params }: Props) {
  ...
  const schema = {"@context":"https://schema.org","@type":"Service",name:`${label} 배관막힘 상담`,provider:{"@type":"LocalBusiness",name:"응급배관119",telephone:"1668-1321"},areaServed:{"@type":"Place",name:`경기도 ${label}`},description:`${label} 싱크대·변기·하수구막힘 원인 점검 및 고압세척 상담`};
  ...
  return <main className="regionPage">
    ...
    <header className="topbar"><a className="brand" href="/"><img className="brandSeal" src="/images/emergency-pipe-stamp.jpeg" alt="응급배관 도장 로고" /><span className="brandText"><span className="brandName">응급배관</span><span className="brandNumber">119</span></span></a><nav><a href="#service">서비스</a><a href="#process">작업절차</a><a href="#work-site">시공현장</a><a href="#faq">자주 묻는 질문</a></nav><a className="headerCall" href="tel:16681321">1668-1321</a></header>
    ...
    <section className="regionHero"><div><p className="eyebrow"><span /> 경기도 {label} 배관 상담</p><h1>{label} 싱크대막힘·변기막힘,<br /><em>원인부터 정확하게 확인합니다</em></h1><p>{profile} 물이 느리게 내려가는지, 한꺼번에 사용할 때 역류하는지, 악취나 소리가 함께 나타나는지를 알려주시면 점검 방향을 안내합니다.</p><div className="heroActions"><a className="primary" href="tel:16681321">{label} 전화 상담 <b>1668-1321</b></a><a className="secondary" href="#symptoms">증상별 점검 보기 ↓</a></div><div className="trust"><span>✓ 작업 전 설명</span><span>✓ 현장 맞춤 장비</span><span>✓ 경기도 지역 상담</span></div></div><figure><img src="/images/plumber-worker.png" alt={`${label} 배관막힘 현장에서 내시경으로 확인하는 작업자`} /><figcaption><small>{label} 배관 점검</small><strong>보이는 증상보다<br />배관 속 원인을 확인합니다.</strong></figcaption></figure></section>
    <section className="dongQuick" id="service"><b>{label} 주요 상담</b><span>싱크대막힘</span><span>변기막힘</span><span>하수구막힘</span><span>배관 내시경</span><span>고압세척</span></section>
    ...
    <footer className="siteFooterV2"><div className="footerBrandV2"><img src="/images/emergency-pipe-stamp.jpeg" alt="응급배관 도장 로고" /><div><span>응급배관</span><b>119</b></div></div><p className="footerServicesV2">싱크대 · 변기 · 하수구 막힘 / 고압세척 · 배관청소</p><p className="footerLegalV2">상담 가능 시간 및 출동 여부는 현장 일정에 따라 달라질 수 있습니다. © 응급배관119</p></footer><a className="floatingCall" href="tel:16681321" aria-label={`${label} 배관 전화 상담`}>☎<b>전화상담</b></a>
  </main>;
}
