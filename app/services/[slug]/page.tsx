...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!service) return {};
  const title = `${service.title} 원인·증상·지역별 안내 | 응급배관119`;
  return {
    title,
...
export default async function ServicePage({ params }: Props) {
  const service = serviceBySlug((await params).slug);
  if (!service) notFound();
  const pageUrl = `https://drain-service119.netlify.app/services/${service.slug}`;
  const schema = [
    {
      ...
      description: service.description,
      url: pageUrl,
      image: `https://drain-service119.netlify.app${service.image}`,
      provider: { "@type": "LocalBusiness", name: "응급배관119", telephone: "1668-1321", url: "https://drain-service119.netlify.app" },
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "배관 서비스", item: "https://drain-service119.netlify.app/services" },
  ...
  return <main className="regionPage serviceDetailPage">
    ...
    <section className="serviceDetailHero">
      <div><p className="eyebrow"><span /> 응급배관119 서비스 안내</p><h1>{service.title},<br /><em>증상과 원인부터 확인합니다</em></h1><p>{service.description}</p><div className="heroActions"><a className="primary" href="tel:16681321">{service.title} 상담 <b>1668-1321</b></a><a className="secondary" href="#regions">지역 페이지 찾기 ↓</a></div></div>
