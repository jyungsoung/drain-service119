import DailyRegistrationDate from "./DailyRegistrationDate";
...
export default function LocalLandingContent({label,leakHref="/leak-detection"}:{label:string;leakHref?:string}){
  return <>
    <section className="serviceShowcase section localFullService"><div className="servicePanel"><div className="serviceHeading"><p className="kicker">DRAIN SERVICE GUIDE</p><h2>{label} 배관막힘 서비스 안내</h2><p>우리동네전문가 대표번호 <b>1668-1321</b> · 막힘과 배관 점검 상담</p></div><div className="servicePhotoGrid drainServicePhotoGrid">{services.map(([title,image,href])=><a className="servicePhotoCard" href={href} key={title} aria-label={`${title} 상세 안내`}><img src={image} alt={`${label} ${title} 서비스`}/><DailyRegistrationDate/><span><strong>{title}</strong><small>지역별 안내 보기</small><b>1668-1321</b></span></a>)}</div><a className="inlineLeakGuide" href={leakHref}><span>{label} 누수 증상은 별도로 확인하세요</span><b>{label} 누수탐지 지역페이지 보기 →</b></a></div></section>
